...
class Frame:
    ...
    def define(self, symbol, value):
        ...
        # define associates a scheme symbol with a value in the current frame
        # BEGIN PROBLEM 1
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 1
        # check if symbol exists in current frame bindings
        if symbol in self.bindings:
            return self.bindings[symbol]
        # if not in current frame, check parent recursively
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 8
        # create a new frame as a child of the current one
        child_frame = Frame(self)
        # iterate over formals and values, binding each pair
        while formals is not nil and vals is not nil:
            # extract the first formal symbol
            symbol = formals.first
            # extract the corresponding value
            value = vals.first
            # define this binding in the child frame
            child_frame.define(symbol, value)
            # move to the next symbol and value
            formals = formals.rest
            vals = vals.rest
        # return the newly created child frame
        return child_frame
        # END PROBLEM 8
